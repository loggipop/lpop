...
describe('Quantum Keys', () => {
  ...
  describe('encrypt and decrypt', () => {
    it('should encrypt and decrypt data successfully', () => {
      ...
      const encrypted = encryptForPublicKey(
        originalData,
    ...
    it('should handle empty string data', () => {
      ...
      const encrypted = encryptForPublicKey(originalData, keys.publicKey);
  ...
  describe('security improvements', () => {
    it('should generate unique ciphertexts for the same plaintext (IV randomization)', () => {
      ...
      const encrypted1 = encryptForPublicKey(originalData, keys.publicKey);
      const encrypted2 = encryptForPublicKey(originalData, keys.publicKey);

      // Same plaintext should produce different ciphertexts due to random IV
      expect(encrypted1.ciphertext).not.toBe(encrypted2.ciphertext);

      // Both should decrypt to the same value
      const decrypted1 = decryptWithPrivateKey(encrypted1, keys.privateKey);
      const decrypted2 = decryptWithPrivateKey(encrypted2, keys.privateKey);
